...
var statistics = {
    ...
    "voteswithpartyR": 0,
    "voteswithID": 0,
    "10percentmembers": sortedslicedmembers,
    "lemisedvotes": sortedslicedmembers4,
    "le%pv": sortedslicedmembers3,
    "memisedvotes": sortedslicedmembers2,
    "me%partyvotes": sortedslicedmembers5,
    "lestloyal": sortedslicedmembers6,
    "lestloyal2": sortedslicedmembers7,
}
...
var sortedmembers6 = members.filter(Boolean).sort((b, a) => a.votes_against_party_pct - b.votes_against_party_pct);
var sortedslicedmembers6 = members.filter(Boolean).slice(0, calculatepercent);
console.log(sortedslicedmembers6);
function mostengaged1 (value){
    for (var i = 0; i < value.length; i++){
    console.log(sortedslicedmembers2[i].first_name)
    ...
    "<td>" + value[i].total_votes+ "</td>" +  

    "<td>" + value[i].votes_against_party_pct+ "</td>" + 

    "</tr>" 
}    
} 
mostengaged1(sortedslicedmembers6);


var sortedmembers7 = members.filter(Boolean).sort((a, b) => a.votes_against_party_pct - b.votes_against_party_pct);
var sortedslicedmembers7 = members.filter(Boolean).slice(0, calculatepercent);
console.log(sortedslicedmembers7);
function mostengaged2 (value){
    for (var i = 0; i < value.length; i++){
    console.log(sortedslicedmembers2[i].first_name)
...
mostengaged2(sortedslicedmembers7);
